...
calculateDistances() {
    ...
    exportClosestBases() {
        ...
        const headers = [
            'Subbase', 'Subbase Address','Subbase Latitude', 'Subbase Longitude',
            'Closest Base', 'Base Address', 'Base Latitude', 'Base Longitude',
            'Distance (mi)'
        ];
        const rows = results.map(row => [
            row.subbaseName, row.subbaseAddress, row.subbaseLat, row.subbaseLon,
            row.baseName, row.baseAddress, row.baseLat, row.baseLon,
            row.distance
        ]);
